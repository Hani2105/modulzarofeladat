...
//        this.eredmenytablaba(indulo, erkezo);
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        String Query = "-- MySQL Script generated by MySQL Workbench\n"
                + "-- Mon Jul 23 09:56:15 2018\n"
                ...
                + "-- Schema telekocsi\n"
                + "-- -----------------------------------------------------\n"
                + "DROP SCHEMA IF EXISTS `telekocsi` ;\n"
                + "\n"
                + "-- -----------------------------------------------------\n"
                + "-- Schema telekocsi\n"
                + "-- -----------------------------------------------------\n"
                ...
                + "  `id_igenyek` VARCHAR(45) NOT NULL,\n"
        ...
        //feltöltjük az autok táblát
        values = "";

        for (int i = 0; i < autok.length; i++) {

            values += "('" + autok[i][2] + "' , '" + autok[i][3] + "'),";

        }

        values = values.substring(0, values.length() - 1);

        Query = "INSERT IGNORE telekocsi.autok (rendszam , telefonszam) values" + values;

        sqlconn conn = null;
        try {
            conn = new sqlconn("com.mysql.jdbc.driver", "jdbc:mysql://localhost/", "root", "");
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(ablak.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SQLException ex) {
            Logger.getLogger(ablak.class.getName()).log(Level.SEVERE, null, ex);
...
//feltöltjük az igények táblát (a már rendelkezésre álló adatbázis adatokból)

       values = "";

       for(int i =1; i < igenyek.length; i++){
       
         Query = "insert into telekocsi.igenyek (id_igenyek , szemelyekszama , indulo_id_varosok , erkezo_id_varosok1) values ('" +igenyek[i][0]+ "' , "+igenyek[i][3]+" , (select telekocsi.varosok.id_varosok from telekocsi.varosok where telekocsi.varosok.varosnev = '"+igenyek[i][1]+"') , (select telekocsi.varosok.id_varosok from telekocsi.varosok where telekocsi.varosok.varosnev = '"+igenyek[i][2]+"'))";
...
//feltöltjük az induloutak táblát (a már rendelkezésre álló adatbázis adatokból)

       for(int i = 1; i< autok.length; i++){
       
          Query = "INSERT INTO telekocsi.induloutak (ferohely , autok_id_autok , indulo_id_varosok , erkezo_id_varosok1) values ("+autok[i][4]+" , (select telekocsi.autok.id_autok from telekocsi.autok where telekocsi.autok.rendszam = '"+autok[i][2]+"' and telekocsi.autok.telefonszam = '"+autok[i][3]+"') , (select telekocsi.varosok.id_varosok from telekocsi.varosok where telekocsi.varosok.varosnev = '"+autok[i][0]+"') , (select telekocsi.varosok.id_varosok from telekocsi.varosok where telekocsi.varosok.varosnev = '"+autok[i][1]+"'))";
